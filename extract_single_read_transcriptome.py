"""
GTR Feature Extraction and Splitting Module for Transcriptome Data
...
Processes both forward and reverse strand BAM files automatically.
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract GTR features from transcriptome BAM files and split by motifs (processes both forward and reverse strands)",
...
Examples:
  # Extract and split by DRACH motifs (default - processes both strands):
  python extract_single_read_transcriptome.py
  
  # Extract and split by all A-centered motifs:
  python extract_single_read_transcriptome.py --motifs all_A_center
  
  # With custom BAM files:
  python extract_single_read_transcriptome.py \\
    --bam-forward path/to/forward.bam \\
    --bam-reverse path/to/reverse.bam
  
  # With custom reference files:
  python extract_single_read_transcriptome.py \\
    --reference-forward path/to/forward_ref.fa \\
    --reference-reverse path/to/reverse_ref.fa
  
  # With custom output directories:
  python extract_single_read_transcriptome.py \\
    --output-forward my_forward_output \\
    --output-reverse my_reverse_output
  
  # Full customization:
  python extract_single_read_transcriptome.py \\
    --bam-forward custom/forward.bam \\
    --bam-reverse custom/reverse.bam \\
    --reference-forward custom/forward_ref.fa \\
    --reference-reverse custom/reverse_ref.fa \\
    --output-forward results_fwd \\
    --output-reverse results_rev \\
    --motifs drach
        """
    )
    parser.add_argument(
        ...
        help="Path to forward strand BAM file (default: alignment/plus_strand/collect.sorted.bam)"
    )
    parser.add_argument(
        "--bam-reverse",
        default='alignment/minus_strand/collect.sorted.bam',
        help="Path to reverse strand BAM file (default: alignment/minus_strand/collect.sorted.bam)"
    )
    parser.add_argument(
        "--output-forward",
        default='output_forward',
        help="Output directory for forward strand (default: output_forward)"
    )
    parser.add_argument(
        "--output-reverse",
        default='output_reverse',
        help="Output directory for reverse strand (default: output_reverse)"
    )
    parser.add_argument(
        "--reference-forward",
        default='alignment/plus_strand/reference.fa',
        help="Path to reference FASTA file for forward strand (default: alignment/plus_strand/reference.fa)"
    )
    parser.add_argument(
        "--reference-reverse",
        default='alignment/minus_strand/reference.fa',
        help="Path to reference FASTA file for reverse strand (default: alignment/minus_strand/reference.fa)"
    )
    parser.add_argument(
        "-p", "--picard",
        default="picard.jar",
    ...
    # Define both strand configurations
    strand_configs = [
        {
            'name': 'Forward Strand',
            'bam_file': args.bam_forward,
            'reference': args.reference_forward,
            'output_dir': args.output_forward
        },
        {
            'name': 'Reverse Strand',
            'bam_file': args.bam_reverse,
            'reference': args.reference_reverse,
            'output_dir': args.output_reverse
        }
    ]
    ...
    for config in strand_configs:
        ...
        result = extract_gtr_features(
            ...
            reference_fasta=config['reference'],
        ...
        if not result:
            print(f"\n✗ Failed to process {config['name']}")
            all_success = False
        else:
            print(f"\n✓ Successfully processed {config['name']}")
    ...
    print("=" * 80)
    if all_success:
        print("✓ Both strands processed successfully!")
        print(f"  - Forward strand: {args.output_forward}/")
        print(f"  - Reverse strand: {args.output_reverse}/")
        sys.exit(0)
    else:
        print("✗ One or more strands failed to process")
        sys.exit(1)